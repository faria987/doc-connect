...
const SpecialitySection =() => {
  return (
    <div className="flex flex-col items-center gap-5 my-20">
      <p className="poppins font-semibold text-3xl">Explore by Specialty</p>
      <p className="text-center mx-10 md:mx-70 poppins font-normal text-gray-500">
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-8 mx-10 md:mx-30 my-10 ">
        {specialityData.map((data, index) => (
          ...
          >
            <img
              className="rounded-full bg-gray-100 p-1 drop-shadow-md"
              key={index}
              src={data.image}
              alt=""
            />
            <p className="poppins text-sm text-center font-medium text-gray-500 mt-3">
              {data.speciality}
            </p>
          </NavLink>
        ))}
